...
void exor_tores(const char kulcs[], int kulcs_meret, 
	char titkos[], int titkos_meret, char* buffer)
{
	char* buffer;

	if ((buffer = (char*)malloc(sizeof(char)*titkos_meret)) == NULL);
{
	printf("Memória probléma a buffernél\n");
	exit(-1)
}
	...
	if(tiszta_lehet(buffer, titkos_meret))
	{
		printf("Kulcs: [%c%c%c%c%c%c%c%c]\nTiszta szöveg: [%s\n]",
							...
							kulcs[4], kulcs[5], kulcs[6], kulcs[7],
							buffer);
	}
